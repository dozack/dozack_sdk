...
static USART_TypeDef *const _uart_USART[UART_INSTANCE_COUNT] = {USART1,
    USART2,
    USART3,
    UART4,
    UART5,
...
static const IRQn_Type _uart_IRQn[UART_INSTANCE_COUNT] = {USART1_IRQn,
    USART2_IRQn,
    USART3_IRQn,
    UART4_IRQn,
    UART5_IRQn,
    ...
    if (_uart->ISR & USART_ISR_RXNE)
    {
        if (_uart->CR1 & USART_CR1_RXNEIE)
        {
            ...
            if (uart->rx_done < uart->rx_size)
            {
                ...
                if ((uart->notify_mode & UART_NOTIFY_MODE_HALF) && (uart->rx_done == (uart->rx_size / 2)))
                {
                    /* If UART_NOTIFY_MODE_HALF mode is enabled, trigger rx event while buffer is half full */
                    events |= UART_EVENT_RECEIVE;
                }
            }
        }
...
bool uart_enable(uart_t *uart, uint8_t *rx_data, uint16_t rx_size, uint32_t btr, uint32_t opt, uart_callback_t cb, uint8_t mode, void *ctx,
                 uint32_t ev) {
    if (uart->state != UART_STATE_INIT)
    {
        return false;
...
bool uart_disable(uart_t *uart) {
    ...
    {
        gpio_pin_configure(uart->pins.cts, (GPIO_PUPD_NONE | GPIO_MODE_ANALOG));
    }
    if (uart->pins.rts_de != GPIO_PIN_NONE)
    {
        gpio_pin_configure(uart->pins.rts_de, (GPIO_PUPD_NONE | GPIO_MODE_ANALOG));
    }
    uart->state = UART_STATE_INIT;
    return true;
}
...
bool uart_configure(uart_t *uart, uint32_t btr, uint32_t opt) {
    ...
    if (((opt & UART_OPTION_DATA_SIZE_MASK) == UART_OPTION_DATA_SIZE_9) && ((opt & UART_OPTION_PARITY_MASK) != UART_OPTION_PARITY_NONE))
    {
        return false;
    }
    if ((opt & (UART_OPTION_CTS | UART_OPTION_RTS)) && (opt & UART_OPTION_DE_MODE))
    {
        return false;
    }
    if (((opt & UART_OPTION_STOP_MASK) == UART_OPTION_STOP_1_5) && (uart->instance == UART_INSTANCE_LPUART1))
    {
        return false;
    }
    ...
    cr2 = 0;
    cr3 = 0;
    switch (opt & UART_OPTION_STOP_MASK) {
    ...
    switch (opt & UART_OPTION_DATA_SIZE_MASK) {
    ...
    switch (opt & UART_OPTION_PARITY_MASK) {
        case UART_OPTION_PARITY_NONE:
            break;
        case UART_OPTION_PARITY_EVEN:
            cr1 |= (USART_CR1_PCE);
        case UART_OPTION_PARITY_ODD:
            cr1 |= (USART_CR1_PCE | USART_CR1_PS);
            break;
        default:
            break;
    }
    if (opt & UART_OPTION_CTS)
    {
        cr3 |= USART_CR3_CTSE;
    }
    if (opt & UART_OPTION_RTS)
    {
        cr3 |= USART_CR3_RTSE;
    }
    if (opt & UART_OPTION_DE_MODE)
    {
        cr3 |= USART_CR3_DEM;
        if (opt & UART_OPTION_DE_ACTIVE_HIGH)
        {
            cr3 |= USART_CR3_DEP;
        }
    }
    ...
    if (uart->pins.cts != GPIO_PIN_NONE)
    {
        gpio_pin_configure(uart->pins.cts, (GPIO_PUPD_NONE | GPIO_OSPEED_VERY_HIGH | GPIO_OTYPE_PUSHPULL | GPIO_MODE_ALTERNATE));
    }

    if (uart->pins.rts_de != GPIO_PIN_NONE)
    {
        gpio_pin_configure(uart->pins.rts_de, (GPIO_PUPD_NONE | GPIO_OSPEED_VERY_HIGH | GPIO_OTYPE_PUSHPULL | GPIO_MODE_ALTERNATE));
    }

    if (cr1 & USART_CR1_RE)
    {
        if (uart->notify_events & UART_EVENT_PARITY)
        {
            cr1 |= USART_CR1_PEIE;
        }
